...
SecAgentX 提供 Web、全屏终端 TUI 和经典 CLI 三种交互方式。安全知识问题使用轻量知识卡片呈现；安全事件则根据风险等级、分析证据和处置需求生成结构化研判结果。自动封禁及其他外部响应操作默认关闭，只有经过用户明确配置后才会启用。
...
按照提示完成模型连接验证，看到“配置完成”后可进入专用终端或启动 Web 对话界面：
...
`secagentx tui` 提供固定的会话记录、对话区、输入区和执行状态栏，支持 `Enter` 发送、`Alt+Enter` 换行、`Ctrl+N` 新建对话、`Ctrl+C` 取消和 `Ctrl+Q` 退出。经典 `secagentx chat` 继续支持 `/new`、`/history`、`/resume`、`/model` 和 `/export` 等命令；自动化脚本可通过 `secagentx ask "问题" --json` 获取机器可读结果。
...
全屏 TUI、经典 CLI 与 Web 服务使用同一套 Agent 和知识库代码，可交互运行，也可输出机器可读 JSON。Web 仅监听本机回环地址，无账户、密码或令牌认证：
...
默认对话区只显示提问、AI 答复和最终研判报告，避免工具调用与 Agent 状态淹没结论。需要审计时，点击工作区右上角的“查看过程”，执行时间线会从右侧覆盖打开，不会挤压或打乱正文布局；最终报告可在“快速分析”和“专家报告”之间切换，并可复制或导出。

回答会按场景自动选择展示形式：问候和一般闲聊使用自然文本；概念、定义和原理类问题（例如“什么是 SQLite 注入？”）显示轻量知识卡片，不附加没有意义的风险评分；配置、加固和合规问题显示操作指南；IP、域名等 IOC 查询显示情报摘要；漏洞、攻击和告警进入安全研判报告；应急处置显示处置报告。后端事件中的 `response_mode` 是这一展示契约，CLI/API 使用者也可据此决定自己的渲染方式。
